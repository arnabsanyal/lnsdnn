...
class fixed_point:
    ...
    def quantize_array(self, _matrix_m1):
        ...
        whole = np.floor(_matrix_m1)
        frac = _matrix_m1 - whole
        frac = frac * self.lshifter
        frac = np.round(frac)               # Maybe round is causing all the mayham
    ...
    def quantize_array_d(self, _matrix_m1):
        return _matrix_m1
...
def main(main_params):
    ...
    if is_training:
        ...
        W2 = fp.quantize_array(np.random.normal(0, 0.1, (101, 10)))
        ...
        np.savez_compressed('./lin_model_MNIST_1_%d_%d.npz' %(qi - 1, qf), W1=W1, W2=W2, loss_train=performance['loss_train'], \
            acc_train=performance['acc_train'], acc_val=performance['acc_val'])

    else:

        file = np.load('./lin_model_MNIST_1_%d_%d.npz' %(qi - 1, qf), 'r')
        W1 = file['W1']
        W2 = file['W2']
        performance = {}
